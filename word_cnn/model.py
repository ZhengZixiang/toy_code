...
def my_model(features, labels, mode, params):
    ...
    embeddings = tf.get_variable(name='embeddings', dtype=tf.float32, shape=[params['vocab_size'], FLAGS.embedding_size])  # shape (vocab, embedding_size) (620228, 128)
    sentence = tf.nn.embedding_lookup(embeddings, sentence)  # shape: (batch, sentence_len, embedding_size) (?, 100, 128)
    # add a channel dim, required by the conv2d and max_pooling2d mathod
    sentence = tf.expand_dims(sentence, -1)  # shape: (batch, sentence_len/height, embedding_size/width, channels=1) (?, 100, 128, 1)

    pooled_outputs = []
    for filter_size in params['filter_sizes']:
        conv = tf.layers.conv2d(sentence,
                                filters=FLAGS.num_filters,
                                kernel_size=[filter_size, FLAGS.embedding_size],
                                strides=(1,1),
                                padding='VALID',
                                activation=tf.nn.relu)  # shape (?,98, 1, 100)
        pool = tf.layers.max_pooling2d(conv,
                                       pool_size=[FLAGS.sentence_max_len - filter_size + 1, 1],
                                       strides=[1, 1],
                                       padding='VALID')  # shape (?, 1, 1, 100)
        pooled_outputs.append(pool)
    h_pool = tf.concat(pooled_outputs, 3)  # shape (?, 1, 1, 300)
    print(h_pool.shape)
    h_pool_flat = tf.reshape(h_pool, [-1, FLAGS.num_filters * len(params['filter_sizes'])])  # shape (?, 300)
    ...
    logits = tf.layers.dense(h_pool_flat, FLAGS.num_classes, activation=tf.nn.softmax)
...
def main(unused_argv):
    ...
    path_vocab = os.path.join(FLAGS.data_dir, 'words.txt')
    assert os.path.isfile(path_vocab), 'No vocab file found at {}, run build_vocab.py'.format(path_vocab)
    ...
    train_spec = tf.estimator.TrainSpec(
        input_fn=lambda: input_fn(path_train, path_vocab, FLAGS.shuffle_buffer_size, config['num_oov_buckets']),
        max_steps=FLAGS.train_steps
    )
    input_fn_for_eval = lambda: input_fn(path_eval, path_vocab, 0, config['num_oov_buckets'])
